...
<div class="content-wrapper">
  ...
  <section class="content">
    <div class="row">
      <div class="col-md-3">
        <a href="<?= base_url('spp/buat') ?>" class="btn btn-primary btn-block mb-3"><i class="fas fa-pen mr-2"></i>Buat Surat SPP Baru</a>
                ...
                <tbody>
                  ...
                  ?>
                    <tr>
                        ...
                        <?php if ($item['status_surat'] == 'Menunggu') : ?>
                          <div class="dropdown">
                            <button class="btn btn-primary btn-sm dropdown-toggle <?= $this->session->userdata('level') != 'Administrator' ? 'disabled' : '' ?>" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                              <?= $item['status_surat'] ?> tanda tangan
                            </button>
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                              <a class="dropdown-item" onclick="return confirm('Apakah anda yakin?')" href="<?= base_url('spp/terima/') . $item['id_spp'] ?>">Selesai Dibuat</a>
                              <a class="dropdown-item" onclick="return confirm('Apakah anda yakin?')" href="<?= base_url('spp/tolak/') . $item['id_spp'] ?>">Tolak Surat</a>
                            </div>
                          </div>
                      ...
                      <td class="text-center">
                        <a href="<?= base_url('spp/detail/') . $item['id_spp'] ?>" class="btn btn-info"><i class="fas fa-eye mr-2"></i>Lihat</a>
                        <a href="<?= base_url('spp/edit/') . $item['id_spp'] ?>" class="btn btn-success"><i class="fas fa-edit mr-2"></i>Edit</a>
                        <a href="<?= base_url('spp/hapus/') . $item['id_spp'] ?>" onclick="return confirm('Apakah anda yakin?')" class="btn btn-danger"><i class="fas fa-trash mr-2"></i>Hapus</a>
                      </td>
                    </tr>
                  <?php endforeach ?>
                </tbody>
