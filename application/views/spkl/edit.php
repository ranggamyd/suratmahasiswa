    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-md-9">
          <div class="card">
            <div class="card-header">
              <!-- <a href="<?= base_url('spkl/edit') ?>" class="btn btn-primary"><i class="fas fa-plus-circle mr-2"></i>Edit SPKL</a> -->
              <h3 class="card-title">Edit Surat Surat Praktek Kerja Lapangan</h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
            <form action="<?= base_url('spkl/proses_edit') ?>" method="post">
            <input type="hidden" name="id" value="<?= $spkl->id_spkl ?>">
              <div class="card-body">
                <div class="form-group">
                  <label for="no_surat">No. Surat</label>
                  <input type="text" name="no_surat" value="<?= $spkl->no_surat ?>" class="form-control bg-light" id="no_surat" placeholder="No. Surat :" readonly required>
                </div>
                <div class="row">
                  <div class="form-group col-md-6">
                    ...
                    <select name="nim_mhs" class="form-control bg-light" id="nim_mhs" required>
                      ...
                      <?php foreach ($mhs as $item) : ?>
                        <option value="<?= $item['nim'] ?>" data-nama_mhs="<?= $item['nama_mhs'] ?>" data-semester="<?= $item['semester'] ?>" data-no_telp="<?= $item['no_telp'] ?>" data-nama_prodi="<?= $item['nama_prodi'] ?>" data-gelar="<?= $item['gelar_kelulusan'] ?>" <?= ($item['nim'] == $spkl->nim_mhs) ? 'selected' : '' ?>>
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <select name="nidn_dekan" class="form-control bg-light" id="nidn_dekan" readonly required>
                      ...
                      <?php foreach ($dekan as $item) : ?>
                        <option value="<?= $item['nidn'] ?>" data-nama_dekan="<?= $item['nama_dekan'] ?>" <?= ($item['nidn'] == $spkl->nidn_dekan) ? 'selected' : '' ?>><?= $item['nama_dekan'] ?></option>
                      <?php endforeach ?>
                    </select>
                  </div>
                </div>
                <div class="row">
                  <div class="form-group col-md-4">
                    ...
                    <input type="text" name="nama_instansi" value="<?= $spkl->nama_instansi ?>" class="form-control bg-light" id="nama_instansi" placeholder="Nama Instansi :" required>
                  </div>
                  <div class="form-group col-md-5">
                    <label for="alamat_instansi">Alamat Instansi</label>
                    <input type="text" name="alamat_instansi" value="<?= $spkl->alamat_instansi ?>" class="form-control bg-light" id="alamat_instansi" placeholder="Alamat Instansi :" required>
                  </div>
                  <div class="form-group col-md-3">
                    <label for="tgl_surat">Tanggal Surat</label>
                    <input type="date" name="tgl_surat" value="<?= $spkl->tgl_surat ?>" class="form-control bg-light" id="tgl_surat" placeholder="Tanggal Surat :" readonly required>
                  </div>
                </div>
                <div class="form-group">
                  <label for="perihal">Perihal</label>
                  <textarea name="perihal" class="form-control bg-light" id="perihal" rows="2" placeholder="Perihal :" required><?= $spkl->perihal ?></textarea>
                </div>
                <div class="form-group">
                  <label for="summernote">Body Surat</label>
                  <textarea name="body_surat" id="summernote" class="form-control bg-light" style="height: 300px" required><?= $spkl->body_surat ?></textarea>
                </div>
              </div>
              <!-- /.card-body -->
