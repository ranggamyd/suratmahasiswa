...
class SC_model extends CI_Model
{
  ...
  {
    $this->db->join('kaprodi', 'kaprodi.nidn = sc.nidn_kaprodi', 'left');
    $this->db->join('mhs', 'mhs.nim = sc.nim_mhs', 'left');
    $this->db->order_by('tgl_surat', 'desc');
    $this->db->order_by('no_surat', 'desc');
    $this->db->order_by('nama_mhs', 'asc');
    if ($this->session->userdata('level') == 'Mahasiswa') {
      return $this->db->get_where('sc', ['nim_mhs' => $this->session->userdata('nim_mhs')])->result_array();
    } else {
      return $this->db->get('sc')->result_array();
    }
  }

  public function get_sc($id)
  {
    $this->db->join('kaprodi', 'kaprodi.nidn = sc.nidn_kaprodi', 'left');
    $this->db->join('mhs', 'mhs.nim = sc.nim_mhs', 'left');
    return $this->db->get_where('sc', ['id_sc' => $id])->row();
  }

  public function count_all()
  {
    return $this->db->get('sc')->num_rows();
  }

  public function count_by_mhs($nim)
  {
    return $this->db->get_where('sc', ['nim_mhs' => $nim])->num_rows();
  }
  ...
  public function set_no_surat()
  {
    ...
    $tahun = date('Y');
    $no_surat_terakhir = $this->db->select('no_surat')->order_by('id_sc', 'DESC')->limit(1)->get('sc')->row('no_surat');
    ...
    if (!empty($no_surat_terakhir)) {
      ...
      $no_surat = '001/1.b/UMC-FT/' . $bulan_romawi . '/' . $tahun;
    }
  ...
  public function edit()
  {
    ...
    $this->db->update('sc', $data, ['id_sc' => $this->input->post('id')]);
  }

  public function hapus($id)
  {
    $this->db->delete('sc', ['id_sc' => $id]);
  }
}
